...
//!   2. let the model reason (`<think>…</think>`) and emit a `<tool_call>`,
...
const MODEL_PATH: &str = "/home/hahihula/mywork/ai/additional_models/Ornith-1.0-9B";
...
fn main() -> Result<()> {
    ...
    eprintln!("Loading Ornith-1.0-9B ({dtype:?}) … this needs ~20 GB on GPU.");
    let mut model = Model::new(MODEL_PATH, &device, &dtype).context("load model")?;
    let tok = AutoTokenizer::from_pretrained(MODEL_PATH, None)

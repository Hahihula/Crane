...
- `kugelaudio_simple.rs`: KugelAudio TTS (Qwen2 backbone + diffusion head, a post-trained VibeVoice fine-tune) — zero-shot or voice cloning from a reference clip via `--ref-wav`. Not wired into `crane-serve` yet, so this is the only way to run it. Needs an external Qwen2-VL-family `tokenizer.json` (the checkpoint ships none) — `crane-model-download --model kugelaudio` fetches both. Runs on CUDA or Metal (`--features cuda`/`--features metal`); `--quant q4_0` (etc., or `CRANE_ISQ`) in-situ-quantizes the ~7B-parameter decoder backbone for low-VRAM/low-RAM setups — the quantization path is covered by unit tests (CPU + Metal, synthetic weights) but a full run of the real ~18.7GB checkpoint hasn't been verified end-to-end on a memory-constrained machine; see that file's doc comment
...
# KugelAudio on macOS: use --features metal instead of --features cuda.
#
# KugelAudio — low-VRAM/low-RAM: 4-bit in-situ quantization of the decoder
# backbone (works with either --features cuda or --features metal). The
# quantization code is unit-tested (CPU + Metal, synthetic weights) but
# loading the real ~18.7GB checkpoint end-to-end has not been verified on a
# memory-constrained machine — expect to need well more than the
# quantized weights' final size free during loading.
cargo run --bin kugelaudio_simple --release --features metal -- \
    checkpoints/kugelaudio-0-open --tokenizer checkpoints/kugelaudio-0-open/tokenizer.json \
    "Text to speak" --quant q4_0

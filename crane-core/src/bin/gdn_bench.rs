...
fn main() -> anyhow::Result<()> {
    anyhow::bail!("gdn_bench needs a GPU backend: build with --features cuda or --features rocm")
}
...
#[cfg(feature = "cuda")]
use crane_core::ops::gdn::gdn_recurrence_cuda as gdn_recurrence;
#[cfg(all(feature = "rocm", not(feature = "cuda")))]
use crane_core::ops::gdn::gdn_recurrence_rocm as gdn_recurrence;
...
fn main() -> anyhow::Result<()> {
    ...
    for _ in 0..10 {
        let _ = gdn_recurrence(&q, &kt, &vt, &g, &beta, &state)?;
    }
    dev.synchronize()?;

    let t = Instant::now();
    for _ in 0..iters {
        let (y, _st) = gdn_recurrence(&q, &kt, &vt, &g, &beta, &state)?;

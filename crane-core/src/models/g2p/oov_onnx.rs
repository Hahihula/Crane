...
//! it. Config and model loading only — no inference yet; wiring this into
//! the lookup chain and running autoregressive decoding happens separately.
...
impl Config {
    ...
    /// `model_kind` don't match what this parser supports, a required
    /// special token is missing from either vocabulary, a `char_vocab` key
    /// (other than a special token) is not exactly one character, or
    /// `phoneme_vocab`'s IDs aren't a contiguous range starting at 0.
    ///

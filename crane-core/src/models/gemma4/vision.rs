...
pub fn preprocess_image(
    ...
    // Cap at 256 to avoid issues with longer sequences
    // Note: num_patches here is already the padded count (max_patches)
    let pk2 = config.pooling_kernel_size * config.pooling_kernel_size;
    let num_real_patches = max_patches.saturating_sub(pad_len);
    let num_image_tokens = (num_real_patches / pk2).min(256);

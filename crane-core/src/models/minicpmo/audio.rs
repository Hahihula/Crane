...
impl AudioProjector {
    ...
    pub fn from_gguf<R: Read + Seek>(gg: &mut Gguf<R>, pool_step: usize) -> Result<Self> {
        let linear1 = Linear::new(
            gg.dequant_tensor("audio_projector.linear1.weight")?,
            Some(
                gg.dequant_tensor("audio_projector.linear1.bias")?
                    .flatten_all()?,
            ),
        );
        let linear2 = Linear::new(
            gg.dequant_tensor("audio_projector.linear2.weight")?,
            Some(
                gg.dequant_tensor("audio_projector.linear2.bias")?
                    .flatten_all()?,
            ),
        );
        Ok(Self {
    ...
    pub fn forward(&self, audio_states: &Tensor) -> Result<Tensor> {
        ...
        hidden
            .narrow(1, 0, usable)?
            .reshape((b, t_pooled, self.pool_step, h))?
            .mean(2)
    }
}
...
mod hf_diff {
    ...
    fn minicpmo_audio_matches_python() {
        ...
        let encoder =
            super::super::audio_encoder::AudioEncoder::new(&config.audio_config, vb.pp("apm"))
                .expect("load apm");
        ...
        let meta: serde_json::Value = serde_json::from_str(
            &std::fs::read_to_string(format!("{diff_dir}/audio_meta.json")).unwrap(),
        )
        .unwrap();
        ...
        let mel_extractor =
            WhisperFeatureExtractor::new(config.audio_config.num_mel_bins, &device, dtype)
                .expect("mel extractor");
        let features = mel_extractor
            .extract(&samples)
            .expect("extract mel features");
        assert_eq!(
            features.real_frame_count,
            meta["mel_frames"].as_u64().unwrap() as usize
        );

        let encoder_out = encoder
            .forward(&features.input_features)
...
mod gguf_cross_check {
    ...
    fn minicpmo_audio_gguf_matches_safetensors() {
        ...
        let gguf_path = std::env::var("CRANE_MINICPMO_AUDIO_GGUF").unwrap_or_else(|_| {
            "/home/hahihula/mywork/ai/additional_models/MiniCPM-o-4_5-audio-F16.gguf".to_string()
        });
        let diff_dir = "/home/hahihula/.claude/jobs/02aa7312/tmp/minicpmo_diff";
        ...
        let st_encoder =
            super::super::audio_encoder::AudioEncoder::new(&config.audio_config, vb.pp("apm"))
                .expect("load apm");
        let st_projector = AudioProjector::new(
            audio_output_dim,
            embed_dim,
            config.audio_pool_step,
            vb.pp("audio_projection_layer"),
        ...
        let gguf_projector = AudioProjector::from_gguf(&mut gg, config.audio_pool_step)
            .expect("load audio projector from gguf");
        ...
        let st_out = st_projector
            .forward(
                &st_encoder
                    .forward(&features.input_features)
                    .expect("st encoder forward"),
            )
            .expect("st projector forward")
            .to_dtype(DType::F32)
            .unwrap();
        let gguf_out = gguf_projector
            .forward(
                &gguf_encoder
                    .forward(&features.input_features)
                    .expect("gguf encoder forward"),
            )
            .expect("gguf projector forward")
            .to_dtype(DType::F32)
            .unwrap();
        ...
        assert!(
            cosine > 0.999,
            "GGUF and safetensors audio outputs diverge too much: cosine {cosine}"
        );
    }
}

...
impl SiglipAttention {
    ...
    fn from_gguf<R: Read + Seek>(
        ...
        let q_proj = Linear::new(
            ...
            Some(
                gg.dequant_tensor(&format!("{prefix}.attn_q.bias"))?
                    .flatten_all()?,
            ),
        );
        let k_proj = Linear::new(
            ...
            Some(
                gg.dequant_tensor(&format!("{prefix}.attn_k.bias"))?
                    .flatten_all()?,
            ),
        );
        let v_proj = Linear::new(
            ...
            Some(
                gg.dequant_tensor(&format!("{prefix}.attn_v.bias"))?
                    .flatten_all()?,
            ),
        );
        let out_proj = Linear::new(
            gg.dequant_tensor(&format!("{prefix}.attn_out.weight"))?,
            Some(
                gg.dequant_tensor(&format!("{prefix}.attn_out.bias"))?
                    .flatten_all()?,
            ),
        );
        Ok(Self {
...
impl Mlp {
    ...
    fn from_gguf<R: Read + Seek>(gg: &mut Gguf<R>, prefix: &str) -> Result<Self> {
        let fc1 = Linear::new(
            ...
            Some(
                gg.dequant_tensor(&format!("{prefix}.ffn_down.bias"))?
                    .flatten_all()?,
            ),
        );
        let fc2 = Linear::new(
            ...
            Some(
                gg.dequant_tensor(&format!("{prefix}.ffn_up.bias"))?
                    .flatten_all()?,
            ),
        );
        Ok(Self { fc1, fc2 })
...
impl VisionEmbeddings {
    ...
    pub fn from_gguf<R: Read + Seek>(gg: &mut Gguf<R>, cfg: &VisionConfig) -> Result<Self> {
        ...
        let patch_embedding = Conv2d::new(
            gg.dequant_tensor("v.patch_embd.weight")?,
            Some(gg.dequant_tensor("v.patch_embd.bias")?.flatten_all()?),
            conv_cfg,
        );
        let position_embedding = Embedding::new(
            gg.dequant_tensor("v.position_embd.weight")?,
            cfg.hidden_size,
        );
        Ok(Self {
            patch_embedding,
    ...
    pub fn forward(&self, pixel_values: &Tensor, tgt_sizes: &[(usize, usize)]) -> Result<Tensor> {
        ...
        let patch_embeds = patch_embeds
            .reshape((b, hidden, max_nb_patches))?
            .transpose(1, 2)?; // [B, max_nb, hidden]
...
impl EncoderLayer {
    ...
    fn from_gguf<R: Read + Seek>(
        ...
        let layer_norm1 = LayerNorm::new(
            gg.dequant_tensor(&format!("{prefix}.ln1.weight"))?,
            gg.dequant_tensor(&format!("{prefix}.ln1.bias"))?,
            cfg.layer_norm_eps,
        );
        let self_attn =
            SiglipAttention::from_gguf(gg, &prefix, cfg.hidden_size, cfg.num_attention_heads)?;
        let layer_norm2 = LayerNorm::new(
            gg.dequant_tensor(&format!("{prefix}.ln2.weight"))?,
            gg.dequant_tensor(&format!("{prefix}.ln2.bias"))?,
            cfg.layer_norm_eps,
        );
...
impl VisionModel {
    ...
    pub fn from_gguf<R: Read + Seek>(gg: &mut Gguf<R>, cfg: &VisionConfig) -> Result<Self> {
        ...
        let post_layernorm = LayerNorm::new(
            gg.dequant_tensor("v.post_ln.weight")?,
            gg.dequant_tensor("v.post_ln.bias")?,
            cfg.layer_norm_eps,
        );
        Ok(Self {
            embeddings,
    ...
    pub fn forward(&self, pixel_values: &Tensor, tgt_sizes: &[(usize, usize)]) -> Result<Tensor> {
        ...
        let attn_mask = if needs_mask {
            ...
            let mask = Tensor::from_vec(mask_data, (b, 1, 1, max_nb_patches), xs.device())?
                .to_dtype(xs.dtype())?;
            Some(mask)
        } else {
            None
...
mod hf_diff {
    ...
    fn minicpmo_vision_matches_python() {
        ...
        let meta: serde_json::Value = serde_json::from_str(
            &std::fs::read_to_string(format!("{diff_dir}/vision_meta.json")).unwrap(),
        )
        .unwrap();
        let tgt_sizes: Vec<(usize, usize)> = meta["tgt_sizes"]
            ...
            .collect();
        let pv_shape: Vec<usize> = meta["pixel_values_shape"]
            .as_array()
            .unwrap()
            .iter()
        ...
        let vision_out = vpm
            .forward(&pixel_values, &tgt_sizes)
            .expect("vision forward");
        let resampled = resampler
            .forward(&vision_out, &tgt_sizes)
            .expect("resampler forward");
        let resampled = resampled.to_dtype(DType::F32).unwrap();

        let rust_shape = resampled.dims().to_vec();
        let expected_shape: Vec<usize> = meta["shape"]
            .as_array()
            .unwrap()
            .iter()
            .map(|v| v.as_u64().unwrap() as usize)
...
mod gguf_cross_check {
    ...
    fn minicpmo_vision_gguf_matches_safetensors() {
        ...
        let gguf_path = std::env::var("CRANE_MINICPMO_VISION_GGUF").unwrap_or_else(|_| {
            "/home/hahihula/mywork/ai/additional_models/MiniCPM-o-4_5-vision-F16.gguf".to_string()
        });
        let diff_dir = "/home/hahihula/.claude/jobs/02aa7312/tmp/minicpmo_diff";
        ...
        let st_resampler = Resampler::new(
            config.query_num,
            embed_dim,
            embed_dim / 128,
            vision_dim,
        ...
        let gguf_resampler = Resampler::from_gguf(&mut gg, embed_dim / 128, vision_dim, embed_dim)
            .expect("load resampler from gguf");
        ...
        let st_out = st_resampler
            .forward(
                &st_vpm
                    .forward(&pixel_values, &tgt_sizes)
                    .expect("st vision forward"),
                &tgt_sizes,
            )
            .expect("st resampler forward")
            .to_dtype(DType::F32)
            .unwrap();
        let gguf_out = gguf_resampler
            .forward(
                &gguf_vpm
                    .forward(&pixel_values, &tgt_sizes)
                    .expect("gguf vision forward"),
                &tgt_sizes,
            )
            .expect("gguf resampler forward")
            .to_dtype(DType::F32)
            .unwrap();
        ...
        assert_eq!(
            st_out.dims(),
            gguf_out.dims(),
            "GGUF and safetensors paths produced different output shapes"
        );
        ...
        assert!(
            cosine > 0.999,
            "GGUF and safetensors vision outputs diverge too much: cosine {cosine}"
        );
    }
}

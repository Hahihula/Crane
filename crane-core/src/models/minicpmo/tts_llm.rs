...
fn undo_llamacpp_rope_permute(w: Tensor, n_head: usize, head_dim: usize) -> Result<Tensor> {
    let in_dim = w.dim(1)?;
    let half = head_dim / 2;
    w.reshape((n_head, half, 2, in_dim))?
        .transpose(1, 2)?
        .contiguous()?
...
impl MiniCpmTts {
    pub fn new(config: &TtsConfig, vb: VarBuilder, device: &Device, dtype: DType) -> Result<Self> {
        ...
        let emb_code = embedding(
            config.num_audio_tokens,
            config.hidden_size,
            vb.pp("emb_code").pp(0),
        )?;
        let projector_semantic = SemanticProjector::new(
            config.llm_dim,
            config.hidden_size,
            vb.pp("projector_semantic"),
        )?;
        ...
        let rotary_emb =
            RotaryEmbedding::new(head_dim, config.max_position_embeddings, 10_000.0, device)?;
    ...
    pub fn from_gguf<R: Read + Seek>(
        ...
        let projector_semantic = SemanticProjector {
            linear1: Linear::new(
                gg.dequant_tensor("projector_semantic.linear1.weight")?,
                Some(
                    gg.dequant_tensor("projector_semantic.linear1.bias")?
                        .flatten_all()?,
                ),
            ),
            linear2: Linear::new(
                gg.dequant_tensor("projector_semantic.linear2.weight")?,
                Some(
                    gg.dequant_tensor("projector_semantic.linear2.bias")?
                        .flatten_all()?,
                ),
            ),
        };
        ...
        let head_code = Linear::new(
            gg.dequant_tensor("head_code.0.weight")?.t()?.contiguous()?,
            None,
        );
        ...
        for i in 0..config.num_hidden_layers {
            ...
            t.insert(
                ...
                undo_llamacpp_rope_permute(
                    gg.dequant_tensor(&format!("{prefix}.attn_q.weight"))?,
            ...
            t.insert(
                ...
                undo_llamacpp_rope_permute(
                    gg.dequant_tensor(&format!("{prefix}.attn_k.weight"))?,
            ...
            t.insert(
                ...
                gg.dequant_tensor(&format!("{prefix}.attn_v.weight"))?,
            );
            t.insert(
                "self_attn.o_proj.weight".into(),
                gg.dequant_tensor(&format!("{prefix}.attn_output.weight"))?,
            );
            t.insert(
                "mlp.gate_proj.weight".into(),
                gg.dequant_tensor(&format!("{prefix}.ffn_gate.weight"))?,
            );
            t.insert(
                "mlp.up_proj.weight".into(),
                gg.dequant_tensor(&format!("{prefix}.ffn_up.weight"))?,
            );
            t.insert(
                "mlp.down_proj.weight".into(),
                gg.dequant_tensor(&format!("{prefix}.ffn_down.weight"))?,
            );
            t.insert(
            ...
            t.insert(
                ...
                gg.dequant_tensor(&format!("{prefix}.ffn_norm.weight"))?,
            );
        ...
        let norm_vb = VarBuilder::from_tensors(
            HashMap::from([(
                "weight".to_string(),
...
impl MiniCpmTts {
    ...
    pub fn generate_chunk(
        ...
        let mut logits = self
            .head_code
            .forward(&hidden.narrow(1, condition_len - 1, 1)?)?
            .squeeze(1)?
            .squeeze(0)?; // [num_audio_tokens]
        ...
        for t in 0..cfg.max_new_tokens {
            ...
            if (cfg.repetition_penalty - 1.0).abs() > f32::EPSILON && !generated.is_empty() {
                step_logits = crate::models::utils::apply_repeat_penalty(
                    &step_logits,
            ...
            let next_embed = self
                .emb_code
                .forward(&Tensor::new(&[next], &self.device)?)?
                .to_dtype(self.dtype)?
                .reshape((1, 1, ()))?;
...
mod hf_diff {
    ...
    fn minicpmo_tts_matches_python() {
        ...
        let filenames =
            crate::utils::utils::get_safetensors_files(model_path).expect("safetensors files");
        ...
        let meta: serde_json::Value = serde_json::from_str(
            &std::fs::read_to_string(format!("{diff_dir}/tts_meta.json")).unwrap(),
        )
        .unwrap();
        let llm_tokens: Vec<u32> = meta["llm_tokens"]
            .as_array()
            .unwrap()
            .iter()
        ...
        let hidden_flat: Vec<f32> = hidden_bytes
            ...
            .collect();
        let llm_hidden_states = Tensor::from_vec(
            hidden_flat,
            (llm_tokens.len(), config.llm.hidden_size),
            &device,
        )
        .unwrap()
        .to_dtype(dtype)
        ...
        let tts_embeds = tts
            .build_condition_embeds(&llm_tokens, &llm_hidden_states)
        ...
        let py_embeds_bytes =
            std::fs::read(format!("{diff_dir}/tts_condition_embeds.bin")).unwrap();
        let py_embeds: Vec<f32> = py_embeds_bytes
            .chunks_exact(4)
            .map(|b| f32::from_le_bytes([b[0], b[1], b[2], b[3]]))
            .collect();
        ...
        assert!(
            embeds_cosine > 0.999,
            "condition_embeds cosine too low: {embeds_cosine}"
        );
        ...
        let py_logits_bytes =
            std::fs::read(format!("{diff_dir}/tts_first_step_logits.bin")).unwrap();
        let py_logits: Vec<f32> = py_logits_bytes
            .chunks_exact(4)
            .map(|b| f32::from_le_bytes([b[0], b[1], b[2], b[3]]))
            .collect();
        ...
        let py_top5: Vec<usize> = meta["top5_codes"]
            .as_array()
            .unwrap()
            .iter()
            ...
            .collect();
        println!(
            "first-step logits cosine: {logits_cosine}, rust top5: {rust_top5:?}, python top5: {py_top5:?}"
        );

        assert_eq!(
            rust_top5[0], py_top5[0],
            "top-1 predicted speech code mismatch"
        );
        assert!(
            logits_cosine > 0.99,
            "first-step logits cosine too low: {logits_cosine}"
        );
    }
}
...
mod gguf_cross_check {
    ...
    fn minicpmo_tts_gguf_matches_safetensors() {
        let model_path = std::env::var("CRANE_MINICPMO_DIR").unwrap_or_else(|_| {
            "/home/hahihula/mywork/ai/additional_models/MiniCPM-o-4_5".to_string()
        });
        let gguf_path = std::env::var("CRANE_MINICPMO_TTS_GGUF").unwrap_or_else(|_| {
            "/home/hahihula/mywork/ai/additional_models/MiniCPM-o-4_5-tts-F16.gguf".to_string()
        });
        let diff_dir = "/home/hahihula/.claude/jobs/02aa7312/tmp/minicpmo_diff";
        ...
        let st_logits = st_tts
            .head_code
            .forward(&st_hidden.narrow(1, st_seq_len - 1, 1).unwrap())
            .unwrap()
            .squeeze(1)
            .unwrap()
            .squeeze(0)
        ...
        let gguf_logits = gguf_tts
            .head_code
            .forward(&gguf_hidden.narrow(1, gguf_seq_len - 1, 1).unwrap())
            .unwrap()
            .squeeze(1)
            .unwrap()
            .squeeze(0)
        ...
        let dot: f64 = st_flat
            ...
            .sum();
        let norm_a: f64 = st_flat
            .iter()
            .map(|a| f64::from(*a) * f64::from(*a))
            .sum::<f64>()
            .sqrt();
        let norm_b: f64 = gguf_flat
            .iter()
            .map(|b| f64::from(*b) * f64::from(*b))
            .sum::<f64>()
            .sqrt();
        let cosine = dot / (norm_a * norm_b);
        let max_abs_diff = st_flat
            .iter()
            .zip(&gguf_flat)
            .map(|(a, b)| (a - b).abs())
            .fold(0f32, f32::max);
        ...
        println!(
            "GGUF vs safetensors first-step logits cosine: {cosine}, max abs diff: {max_abs_diff}"
        );
        ...
        assert!(
            cosine > 0.999,
            "GGUF and safetensors tts outputs diverge too much: cosine {cosine}"
        );
    }
}

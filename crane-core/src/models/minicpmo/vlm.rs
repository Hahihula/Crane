...
use super::audio_encoder::{AudioEncoder, pooled_output_len};
use super::config::{MiniCpmOConfig, load_config};
use super::llm::MiniCpmOLlm;
use super::preprocess::{
    self, PreprocessorConfig, ProcessedImage, build_placeholder, load_preprocessor_config,
};
...
impl MiniCpmOVlModel {
    pub fn new(model_path: &str, device: &Device, dtype: &DType) -> Result<Self> {
        ...
        let vision =
            VisionModel::new(&config.vision_config, vb.pp("vpm")).context("build vision tower")?;
        ...
        let audio_encoder =
            AudioEncoder::new(&config.audio_config, vb.pp("apm")).context("build audio encoder")?;
        let audio_output_dim = config.audio_config.encoder_ffn_dim / 4;
        let audio_projector = AudioProjector::new(
            audio_output_dim,
            embed_dim,
    ...
    pub fn encode_images(
        ...
        for &(h, w) in target_sizes {
            ...
            let row = if n < max_patches {
                let pad = Tensor::zeros(
                    (1, c, patch_size, (max_patches - n) * patch_size),
                    row.dtype(),
                    device,
                )?;
        ...
        let vision_out = self
            .vision
            .forward(&batched, target_sizes)
            .context("vision forward")?;
        self.resampler
            .forward(&vision_out, target_sizes)
            .context("resampler forward")
    }
    ...
    pub fn encode_audio(&self, samples: &[f32]) -> Result<Tensor> {
        ...
        let encoder_out = self
            ...
            .context("audio encoder forward")?;
        self.audio_projector
            .forward(&encoder_out)
            .context("audio projector forward")
    }
    ...
    pub fn forward(
        ...
        if !placeholder_embeds.is_empty() {
            ...
            hidden_states =
                splice_image_features(input_ids, &hidden_states, &combined, self.unk_token_id)?;
        }

        self.llm
            .forward_embeds(&hidden_states, start_pos)
    ...
    pub fn decode_step(&mut self, token: u32, start_pos: usize) -> Result<Tensor> {
        ...
        self.llm
            .forward_embeds(&hidden, start_pos)
            .map_err(anyhow::Error::from)
    }
    ...
    fn render_prompt_for_tts(
        ...
        let tts_bos = self
            .tts_bos_token_id
            .context("tokenizer has no <|tts_bos|> token")?;
    ...
    pub fn generate_for_tts(
        ...
        for _ in 0..cfg.max_new_tokens {
            ...
            let hidden = self
                .llm
                .last_hidden_states()
                .context("no hidden states after forward")?
                .clone();

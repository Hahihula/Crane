...
fn assert_well_formed(wav: &candle_core::Tensor, label: &str) {
    ...
    assert!(
        ...
        "{label}: sample outside tanh range"
    );
...
fn voxcpm2_continuation_mode_voice_clone_is_well_formed() {
    ...
    let ref_wav = std::env::var("CRANE_VOXCPM2_REF_WAV")
        .expect("set CRANE_VOXCPM2_REF_WAV to a reference wav");
    ...
    let prompt_feat = model
        .encode_reference_audio(&samples, true)
        .expect("encode_reference_audio");
    ...
    let wav = model
        .generate_speech_conditioned(
            "Now the cloned voice says something new.",
            &conditioning,
            &cfg,
        )
...
fn voxcpm2_prompt_cache_build_merge_generate_round_trips() {
    ...
    let prompt_feat = model
        .encode_reference_audio(&samples, true)
        .expect("encode_reference_audio");
    let cache = VoxCpm2PromptCache::build(
        None,
        Some(("Reference transcript.".to_string(), prompt_feat)),
    )
    ...
    let (wav1, gen_feat1) = model
        .generate_with_prompt_cache(turn1_text, Some(&cache), &cfg)
        .expect("generate_with_prompt_cache turn 1");
    assert_well_formed(&wav1, "prompt-cache turn 1");

    let merged = model
        .merge_prompt_cache(Some(&cache), turn1_text, &gen_feat1)
        .expect("merge_prompt_cache");
    assert!(merged.prompt_text.ends_with(turn1_text));

    let (wav2, _gen_feat2) = model
        .generate_with_prompt_cache(
            "Second generated turn, from the merged cache.",
            Some(&merged),
            &cfg,
        )

...
fn minicpmo_duplex_session_arbitrates_listen_speak() {
    ...
    let audio_path = std::env::var("CRANE_MINICPMO_TEST_AUDIO")
        .unwrap_or_else(|_| format!("{dir}/assets/haimianbaobao.wav"));

    let mut reader =
        hound::WavReader::open(&audio_path).unwrap_or_else(|e| panic!("open {audio_path}: {e}"));
    ...
    for (i, chunk) in samples.chunks(chunk_len).enumerate() {
        ...
        let out = session
            .streaming_generate(42 + i as u64)
            .expect("streaming_generate");
        println!(
        ...
        if let Some(wav) = &out.audio_waveform {
            ...
            assert!(
                wav.iter().all(|v| v.is_finite()),
                "non-finite sample in turn audio"
            );
            got_audio = true;
        }
...
fn minicpmo_duplex_session_sliding_window_shrinks_cache() {
    ...
    for (i, chunk) in samples.chunks(chunk_len).cycle().take(30).enumerate() {
        ...
        println!(
            "chunk {i}: cache_len={len} is_listen={} end_of_turn={}",
            out.is_listen, out.end_of_turn
        );
        ...
        assert!(
            len >= system_preserve,
            "cache must never shrink below the protected system-prompt prefix"
        );
    }
    ...
    assert!(
        max_len_seen > 120,
        "test didn't grow the cache enough to exercise eviction at all — increase chunk count"
    );
    assert!(
        eviction_observed,
        "basic-mode sliding window should have shrunk the cache at least once over 30 chunks with a 200/120 watermark"
    );
}
...
fn minicpmo_duplex_session_context_preserve_rebuilds_cache() {
    ...
    for (i, chunk) in samples.chunks(16_000).cycle().take(15).enumerate() {
        ...
        assert!(
            len > 0,
            "session should still be able to generate after a context-mode rebuild"
        );
    }

    println!("final cache length: {last_len}, shrank at least once: {shrank_at_least_once}");
    assert!(
        shrank_at_least_once,
        "context-preserve mode should have rebuilt (and shrunk) the cache at least once over 15 chunks with context_max_units=3"
    );
}
...
fn minicpmo_duplex_session_gguf_llm_arbitrates_listen_speak() {
    ...
    let dir = std::env::var("CRANE_MINICPMO_DIR")
        .expect("set CRANE_MINICPMO_DIR to a MiniCPM-o-4.5 checkpoint dir");
    let gguf_path = std::env::var("CRANE_MINICPMO_LLM_GGUF")
        .expect("set CRANE_MINICPMO_LLM_GGUF to a standalone MiniCPM-o-4.5 LLM GGUF file");
    let audio_path = std::env::var("CRANE_MINICPMO_TEST_AUDIO")
        .unwrap_or_else(|_| format!("{dir}/assets/haimianbaobao.wav"));
    ...
    let mut session = DuplexSession::new_with_llm_gguf(&dir, &gguf_path, &device, dtype, cfg)
        .expect("load duplex session with GGUF LLM");
    session
        .prepare(Some(
            "You are a friendly voice assistant having a live spoken conversation.",
        ))
...
fn minicpmo_duplex_session_all_gguf_towers_arbitrate_listen_speak() {
    ...
    let audio_gguf =
        std::env::var("CRANE_MINICPMO_AUDIO_GGUF").expect("set CRANE_MINICPMO_AUDIO_GGUF");
    ...
    let gguf = DuplexGgufPaths {
        llm: Some(&llm_gguf),
        audio: Some(&audio_gguf),
        tts: Some(&tts_gguf),
    };
    let mut session = DuplexSession::new_with_gguf(&dir, &gguf, &device, dtype, cfg)
        .expect("load duplex session with all GGUF towers");
    session
        .prepare(Some(
            "You are a friendly voice assistant having a live spoken conversation.",
        ))
    ...
    for (i, chunk) in samples.chunks(chunk_len).enumerate() {
        ...
        let out = session
            ...
            .expect("streaming_generate");
        println!(
            "chunk {i}: is_listen={} end_of_turn={} text={:?}",
            out.is_listen, out.end_of_turn, out.text
        );
        if let Some(wav) = &out.audio_waveform {
            assert!(
                wav.iter().all(|v| v.is_finite()),

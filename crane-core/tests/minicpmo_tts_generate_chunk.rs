...
fn minicpmo_tts_generate_chunk_persists_kv_across_calls() {
    ...
    assert!(
        response_tokens.len() >= 4,
        "need at least a few tokens to split into two meaningful chunks"
    );
    ...
    let condition_a = tts
        .build_condition_embeds(tokens_a, &hidden_a_cat)
        .expect("chunk 1 condition embeds");
    ...
    let cfg_chunk = TtsGenerationConfig {
        min_new_tokens: 0,
        max_new_tokens: 26,
        ..Default::default()
    };
    let codes_a = tts
        .generate_chunk(&inputs_a, 0, &cfg_chunk)
        .expect("generate_chunk 1");
    println!(
        "chunk 1: {} codes from {} condition positions",
        codes_a.len(),
        len_a
    );
    ...
    let condition_b = tts
        .build_condition_embeds(tokens_b, &hidden_b_cat)
        .expect("chunk 2 condition embeds");
    ...
    let cfg_final_chunk = TtsGenerationConfig {
        min_new_tokens: 0,
        max_new_tokens: 200,
        ..Default::default()
    };
    let codes_b = tts
        .generate_chunk(&inputs_b, tts_start_pos_b, &cfg_final_chunk)
        .expect("generate_chunk 2 (continues chunk 1's KV cache)");
    println!(
        "chunk 2: {} codes from {} condition positions (continuing from position {tts_start_pos_b})",
        codes_b.len(),
        len_b
    );
    ...
    assert!(
        !codes_a.is_empty() || !codes_b.is_empty(),
        "TTS produced no speech tokens across either chunk"
    );
    ...
    let out_path = std::env::var("CRANE_MINICPMO_CHUNK_SPEECH_OUT")
        .unwrap_or_else(|_| "/tmp/minicpmo_tts_chunk_test.wav".to_string());

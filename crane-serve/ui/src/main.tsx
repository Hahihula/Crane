...
function App() {
  ...
  return <main className="min-h-screen bg-white text-zinc-900"><header className="flex h-14 items-center justify-between border-b border-zinc-200 px-6 md:px-[max(24px,calc((100vw-960px)/2))]"><div className="flex items-center gap-2 text-[17px] font-semibold tracking-tight"><span className="text-xl leading-none">⌁</span>Crane</div><span className="max-w-[52vw] truncate text-xs text-zinc-500">{config.model_name} · {config.model_type}</span></header>{config.mode === 'asr' ? <AsrView config={config} /> : config.mode === 'tts' ? <TtsView config={config} /> : <ChatView config={config} />}</main>
}
